...
namespace WebApplication3.Controllers
{
...

    {
        ...
        public void DeleteCourses(List<int> courseId)
        {
            ...
            // Obtener los cursos a eliminar
            var coursesToDelete = _context.Course.Where(c => courseId.Contains(c.IdCourse)).ToList();

            if (coursesToDelete.Count == 0)
            {
                throw new ArgumentException("No se encontraron identificadores");
            }

            // Eliminar los cursos
            _context.Course.RemoveRange(coursesToDelete);
            _context.SaveChanges();
        }
